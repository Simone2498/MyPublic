...
#Overall counting variables usefull only for the tests
n_iterations=[0]
n_split=[0]
n_update=[0]
n_pivot=[0]
stack_list=[0]
TEST_MODE=False #Change to true for running in test mode and print the counting variables
...
class Reluplex(object):
    ...
    def __active_node(self, relu_pair):
      #make the relu_pair relation active, this means relu constraints mantained and node_n limited from 0 to inf
      node_n=relu_pair[0]
      node_f=relu_pair[1]
      self.value_constraints[node_n]=[0, inf]

    def __deactivate_node(self, relu_pair):
      #for deactivate a relu pair I need to switch off the node_f (set to zero), and make the correspective node_n free to move under the 0 bound. I have to broke their relu relation too
      node_n=relu_pair[0]
      node_f=relu_pair[1]
      self.value_constraints[node_n]=[_inf, 0]
    ...
    def new_run(self, debug_mode):
        ...
        for t in range(self.T): #T treshold
          ...
          if k1 is True: #if all the values are in their bounds
            ...
            node_b = relu_pair[0] # I consider the node_n
...
if __name__ == "__main__":
  ...
  test_instance = Reluplex(layers, 15000)
  #set all the constraints manually or using a list of them
  test_instance.set_bounds([1,0],[0,1]) #this means node 0 of the layer 1 needs to be between 0 and 1
  test_instance.set_bounds([3,0],[0.5,1]) #layers start from 1, nodes in layer start from 0, it's possible to set only n nodes
  #run the verification, with or without (False) the report printing
  print(test_instance.new_run(False)) #this print the outcome
